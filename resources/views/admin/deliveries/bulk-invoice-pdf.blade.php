...
<body>
    ...
    @foreach ($bookings as $booking)
        <div class="label">
            ...
            <table width="100%" cellspacing="0" cellpadding="0">
                <tr>
                    <td width="22%" valign="top">
                        @if (!empty($booking->Merchant?->image))
                    ...
                    <td width="56%" valign="top" style="padding:0 2px;">
                        <div class="store-name">
                            {{ Str::limit($booking->store->name ?? 'N/A', 22) }}
                        </div>
                        <div class="small">
                            {{ Str::limit($booking->store->address ?? 'N/A', 35) }}
                        </div>
                        <div class="small">
                            {{ $booking->store->primary_phone ?? 'N/A' }}
                        </div>
                    </td>
            ...
            <!-- RECIPIENT INFO -->
            <table width="100%" cellspacing="0" cellpadding="0"
                style="margin-top:3px; border-collapse: separate; border-spacing: 0 5px;">
                <tr>
                    <td class="recipient-name">
                        {{ Str::limit($booking->recipient_name ?? 'N/A', 28) }}
                    </td>
                </tr>

                <tr>
                    <td>
                        <span class="small">{{ $booking->recipient_phone ?? 'N/A' }}</span>
                        &nbsp;
                        <span class="cod">
                ...
                <tr>
                    <td class="small">
                        {{ Str::limit($booking->recipient_address ?? 'N/A', 70) }}
                    </td>
                </tr>
            </table>
