...
@section('content')
    ...
    <div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Manage District</div>
        <div class="ps-3">
        ...
        <div class="ms-auto">
            <a href="{{ route('admin.district.create') }}" class="btn btn-sm btn-dark">
        ...
        <div class="col-lg-12">
            <div class="card shadow-sm">
                ...
                <div class="card-body table-responsive">
                    {{-- Search --}}
                    <form method="GET" action="{{ route('admin.district.index') }}" class="mb-4">
                        ...
                        <tbody>
                            @foreach ($districts as $index => $district)
                                <tr>
                                    ...
                                    <td>{{ $district->division->name ?? null }}</td>
                                    <td>{{ $district->name }}</td>
                                    <td>
                                        <a href="{{ route('admin.district.toggle.status', $district->id) }}"
                                            class="btn btn-sm d-inline-flex align-items-center {{ $district->status ? 'btn-success' : 'btn-danger' }}"
                                            onclick="return confirm('Are you sure you want to toggle status?')">
                                            <i
                                                class="bx {{ $district->status ? 'bx-toggle-right' : 'bx-toggle-left' }} me-1"></i>
                                        ...
                                        <a href="{{ route('admin.district.delete', $district->id) }}"
