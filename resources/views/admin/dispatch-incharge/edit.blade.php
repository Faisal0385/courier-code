...
@section('content')
    <div class="container-fluid">
        <div class="row mb-3">
            <div class="col-lg-12">
                <a class="btn btn-sm btn-outline-primary" href="{{ route('admin.dispatch.incharge.index') }}">
                    <i class="fa fa-arrow-left"></i> Back to Dispatch Incharge List
                </a>
            </div>
        </div>
        <hr>
            ...
            <div class="col-lg-8">
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-success text-white">
                        <h5 class="mb-0 text-white"><i class="fas fa-folder-plus me-2"></i> Edit Dispatch Incharge</h5>
                    </div>

                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.dispatch.incharge.update', $hubIncharge->id) }}">
                            @csrf
                                    ...
                                    <select class="form-select form-select-md" name="hub_id"
                                        ...
                                        @foreach ($hubLists as $hubList)
                                            <option value="{{ $hubList->id }}"
                                                {{ $hubIncharge->hub_id == $hubList->id ? 'selected' : '' }}>
                                                {{ $hubList->name }}
                                            </option>
                                        @endforeach
                                    </select>
